...
const SidePanel = () => {
  ...
  return (
    <Container fluid="md" className={`${width > 550 ? 'h-100 px-20 py-16' : 'px-8 py-4'} `} style={{ backgroundColor: "#252424" }}>
      <Row className={`${width > 550 ? 'h-100 flex-column' : ''}  justify-content-between`}>
        <Col className={`${width > 550 ? `align-items-start justify-content-between` : `justify-content-center`} d-flex flex-column`}>
          ...
          <Row >
            <Col style={{ marginBottom: `${width > 550 ? "-50px" : ''}` }}>
        ...
        <Col className={`${width > 550 ? 'align-items-end' : 'justify-content-center'} d-flex `}>
          <Button className={`${width > 500 ? 'd-flex align-items-center py-2 px-6' : 'px-4 pt-8 '} bg-warning fs-6 border-0`} style={{ width: `${width > 550 ? '' : '130px'}`, height: `${width > 550 ? '' : '130px'}`, borderRadius: `${width > 550 ? '30px' : '50%'}` }}>
            Получить консультацию
            {width > 550 ? `&rarr;` : <p className={`${width > 500 ? ' align-items-center justify-content-center' : ''} h-100 `}>&rarr;</p>}
          </Button>
        </Col>
      </Row>
    </Container>
